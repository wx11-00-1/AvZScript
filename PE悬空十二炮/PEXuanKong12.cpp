...
#define _DOOM_SHROOM 9

// #define target_flag 89998 // 目标旗帜数
...
// 全局变量 记录已完成轮数
// class FlagCounter : AvZ::GlobalVar
// {
// public:
//     int flag_cout = 0;
...
void Script()
{
    ...
    auto condition = [=]()
    {
        ...
        // for (int index = 0; index < AvZ::GetMainObject()->zombieTotal(); ++index)
        // {
        //     if (zombie_array[index].isExist() && !zombie_array[index].isDead())
        //     {
        //         int col = zombie_array[index].abscissa() / 80;
        //         if (col <= 6)
        //         {
        //             int zT = zombie_array[index].type();
    ...
    auto callback = [=]()
    {
        ShowErrorNotInQueue("气球快进家了或春哥无了，嘤嘤嘤");
    };
    ...
    InsertOperation([=]()
                    ...
                    {
                        SetTime(852, 10);
                        // 根据当前场地物品数量 判断是否存在未消失的弹坑
                        // int item_cout=*reinterpret_cast<int*>(*reinterpret_cast<int*>(*reinterpret_cast<int*>(0x6a9ec0) + 0x768) + 0x12c);
                        SmartCard(_DOOM_SHROOM, 2, 8);
                        SmartCard(_COFFEE, 2, 8);
                        SmartCard(_DOOM_SHROOM, 5, 8);
                        SmartCard(_COFFEE, 5, 8);
                    } });
